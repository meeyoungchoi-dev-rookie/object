...
- 객체지향에서는 타입 변수를 활용한 조건문을 다형성으로 대체한다
- 객체가 메시지를 처리할 적절한 메서드를 선택한다
- 조건문을 사용하는 것을 기피하는 이유
- 변경이 발생하면 조건문 내부 코들르 수정해야 하기 때문이다
...
- 추상 데이터 타입은 타입을 추상화한 것이다
- 클래스는 절차를 추상화한 것이다

- 추상화 - 하나의 대표적인 타입이 다수의 세부적인 타입을 감추는 경우
- 즉 , 개별 오퍼레이션은 모든 개념적인 타입에 대한 구현을 포괄하여 하나의 물리적인 타입 안에 전체 타입을 감춘다

- 객체지향
- 타입을 기준으로 오퍼레이션을 묶는다
- 공통 로직을 포함할 부모 클래스를 정의한다
- 자식 클래스가 부모 클래스를 상속 받는다
- 클라이언트는 부모 클래스의 참조자에 대해 메시지를 전송하면 실제 클래스가 무엇인가에 따라 적절한 절차가 시행된다
- 즉 , 동일한 메시지에 대해 서로 다르게 반응하게 된다

실제 내부에서 수행되는 절차는 다르지만 클래스를 이용한 다형성은 절차에 대한 차이점을 감춘다

### 개방 - 폐쇄 원칙

- 기존 코드에 아무런 영향도 미치지 않고 새로운 객체 유형과 행위를 추가할 수 있는 방법
- 새로운 타입을 자주 추가해야 한다면 객체지향의 클래스 구조가 더 유용하다
- 새로운 오퍼레이션을 추가해야 한다면 추상 데이터 타입을 선택하는 것이 좋다
...
- 의존성은 객체들의 협력을 가능하게 만드는 매개체라는 관점에서 바람직 하다
- 하지만 의존서잉 과하면 문제가 될수 있다
- 바람직한 의존성
- 어떤 의존성이 다양한 환경에서 클래스를 재사용할 수 없도록 제한한다면 그 의존성은 바람직하지 못한 것이다
- 어떤 의존성이 다향한 환경에서 재사용할 수 있다면 그 의존성은 바람직한 것이다
- 어떤 두 요소 사이에 존재하는 의존성이 바람직 할 때 두 요소가 느슨한 결합도 또는 약한 결합도를 가진다고 말한다

<aside>
📌 바람직한 의존성이란 설계를 재상요하기 쉽게 만드는 의존성이다

</aside>


### 추상화에 의존하라
- `추상화` - 세부사항을 더 명확하게 이해하기 위해 특정 절차를 생략하거나 감추으로써 복잡도를 극복하는 방법
- 대상에 대해 알아야 하는 지식의 양을 줄 일수 이씩 떄문에 결합도를 느슨하게 유지할 수 있다

- 추상화와 결합도의 관점에서 의존 대상을 아래 3가지로 구분하는 것이 유용하다
  - 구체 클래스 의존성
  - 추상 클래스 의존성
  - 인터페이스 의존성
...
- 하지만 추상 클래스의 클라이언트는 여전히 협력하는 대상이 속한 클래스 상속 계층이 무엇인지에 대해 알고 있어야 한다

- 인터페이스에 의존하면 상속 계층을 모드러라도 협력이 가능해 진다
- 인터페이스 의존성은 협력하는 객체가 어떤 메시지를 수신할 수 있는지에 대한 지식만을 남기기 때문에 추상 클래스 의존성보다 결합도가 낮아진다
...
- 중복 할인 정책을 구현하는 OrverlappedDiscountPolicy 클래스를 추가하더라도 Movie 클래스는 여전히 DiscountPolicy 클래스에만 의존한다
- 컴파일타임 의존성은 변하지 않는다
- 런타임에 Movie 객체는 OverlappedDiscountPolicy 인스턴스와 협력할 수 있다
- 따라서 런타임 의존성은 변경된다
- 의손정 관점에서 개방 - 패쇄 원칙을 따르는 설계란 컴파일타임 의존성은 유지하면서 런타임 의존성의 가능성을 확장하고 수정할 수 있는 구조를 의미한다

### 추상화가 핵심이다

- `추상화` - 핵심적인 부분만 남기고 불필요한 부분은 생략 , 복잡성을 극복한다
- 역할이 바뀌더라도 협력시 변하지 않는 부분만 님긴다
- 역할에 따라 변하는 부분은 생략된다
- 추상화를 사용하면 역할에 따라 기능을 구체화 하고 확장할 수 있다

<aside>
📌 변하지 않는 부분을 고정하고 변하는 부분을 생략하는 추상화 메커니즘이 개방-폐쇄 원칙에 기반된다

</aside>

# 생성 사용 분리

- 유연하고 재사용 가능한 설계를 위해 객체에 대한 생성과 사용을 분리해야 한다
- 객체를 생성할 책임을 클라이언트로 옮긴다

## FACTORY 추가하기

- 생성과 사용을 분리하기 위해 객체 생성에 특화된 객체를 FACTORY 라고 부른다
- Movie를 사용하는 클라이언트도 Movie의 인스턴스를 생성하는 동시에 getFee 메시지도 함께 전송한다
- 클라이언트 역시 생성과 사용의 책임을 함께 지고 있는 것이다
- 객체 생성과 관련된 지식이 클라이언트와 협력하는 클라이언트에게 까지 새어나가기를 원하지 않는 경우
- 객체 생성과 관련된 책임만 전담하는 별도의 객체를 추가하고 클라이언트는 해당 객체를 사용하도록 만들 수 있다
...
- FACTORY를 사용하면 Movie와 AmountDiscountPolicy를 생성하는 책임을 FACTORY로 이동할 수 있따
- Client엔느 사용과 관련된 책임만 남게 되는데 하나는 FACTORY를 통해 생성된 Movie 객체를 얻기위한 것이다
- 또한 Movie를 통해 가격을 계산할 수 있다

<aside>
📌 결합도를 낮추고 재사용성을 높이기 위해 도메인 개념에게 할당돼 있던 객체 생성 책임을 도메인 개념과는 상관없는 가공의 객체로 이동시켰다
...
- 책임ㅇ르 할당하기 위해 창조되는 모데인과 무ㅘㄴ한 인공적인 객체를 순수한 가공물 이라고 한다
- 모든 책임을 도메인 객체에게 할다앟면 낮은 응집도, 높은 결합도, 재사용성 저하와 같은 심각한 문제점에 봉착하게 될 가능성이 커진다
- 도메인 개념을 표현한 객체가 아닌 설계자가 편의를 위해 임의로 만들어낸 가공으이 객체에게 책임을 할당해서 문제를 해결해야 한다


## 의존성 주입

- 외부에서 의존성의 대상을 해결한 후 이를 사용하는 객체 쪽으로 주입하는 방식
...
- 의존성을 숨기는 코드는 단위 테스트 작서잉 어렵다
- 의존성을 이해하기 위해 코드의 내부 구현을 이해해야 한다
- 따라서 숨겨진 의존성은 캡슐화를 위반하게 된다
- 숨겨진 의존성은 의존성의 대상을 설정하는 시점과 의존성이 해결되는 시점을 떨어뜨려 놓는다
...
### 추상화와 의존성 역전
- 객체 사이의 협력이 존재할때
- 협력에서 중요한 정책이나 의사결정 , 비즈니스의 본질을 담고 있는 클래스는 상위 수준의 클래스이다
- 상위 수준 클래스가 하위 수준 클래스에 의존하면 하위 클래스가 변경되면 상위 클래스도 영향을 받게 된다
- 하위 클래스인 AmountDiscountPolicy를 PercentDiscountPolicy로 변경한다고 해도 Movie 클래스가 영향을 받으면 안된다

<aside>
📌 의존성은 변경의 전파와 관련되 있다
설계는 변경의 영향을 최소화 하도록 의존성을 관리해야 한다
...
### 추상화
- 하위 수준 클래스의 변경으로 인해 사우이 수준의 클래스가 영향을 받는 것을 방지할 수 있다
- 또한 상위 수준 클래스를 재사용할 때 하위 수준의 클래스에 얽매이지 않고 다양한 컨텍스트에서 재사용이 가능하다

<aside>
📌 유연하고 재사용 가능한 설계를 위해 추상화에 의존해야 한다
...
![인터페이스가 서버 모듈 패키지에 위치해 있는 경우 문제점](https://user-images.githubusercontent.com/42866800/165312026-186c7c5b-1eea-4a91-a636-cc862066a07c.png)
  
- DiscountPolicy가 포함돼 있는 패키지 안에 AmoundDiscountPolicy 클래스와 PercentDiscountPolicy 클래스가 포함돼 있다
- DiscountPolicy 클래스에 의존하기 위해서는 반드시 같은 패키지에 포함된 AmounDiscountPolicy 클래스와 PercentDiscountPolicy 클래스도 함께 존재해야 한다는 것을 의미한다

- DiscountPolicy가 포함된 패키지 안의 어떤 클래스가 수정되더라도 패키지 전체가 재배포 돼야 한다
...
- 따라서 추상화를 클라이언트가 속한 패키지에 포함시켜야 한다
- 함께 재사용될 필요가 없는 클래스는 별도의 독립적인 패키지에 모아야 한다
![인터페이스의 소유권을 역전시킨 모듈 구조](https://user-images.githubusercontent.com/42866800/165312147-1bfd3d48-8c43-4034-9605-74dd0fd38863.png)

### 유연한 설계는 유연성이 필요할 때만 옳다

- 변경하기 쉽고 확장하기 쉬운 구조를 만들기 위해 단순함과 명확함의 미덕을 버리게 될 가능성이 높다
- 설계가 유연할 수록 클래스 구조와 객체 구조 사이의 거리는 점점 멀어진다

<aside>
📌 유연성은 코드를 읽는 사람이 복잡함을 수용할 수 있을 때만 가치가 있다

</aside>

- 따라서 협력을 재사용할 필요가 없다면 설계를 유연하게 만들 이유도 사라진다
- 객체들이 동일한 책임을 수행하는지 여부를 판단할 수 없다면 공통의 추상화를 도출할 수 없다
- 왜?
- 동리한 역할을 통해 객체들이 대체 가능하지 않기 때문에 협력에 참여하는 객체를 교체할 필요가 없어지기 때문이다

<aside>
📌 비즈니스 로직을 처리하기 위해 책임을 할당하고 협력의 균형을 맞추는 것이 객체 생성에 관한 책임을 할당하는 것보다 중요하다

</aside>

- 객체를 생성하는 방법에 대한 결정느 모든 책임이 자리를 잡은 후 가장 마지막 시점에 내리는 것이 적절하다